...
function ListCertificate(props) {
  ...
  const editItem = (item) => {
    ...
    setDateCertifiCateUpdate(new Date(item.certiDate));
    setIsEditRow(true);
  };
  const editRow = (item) => {
    CertificateService.updateCertificated(
      ...
      (response) => {
        ...
        let newArr = [...listCertificate];
        for (let i = 0; i < newArr.length; i++) {
          if (newArr[i].id === idCertificateUpdate) {
            newArr[i].name = nameCertificateUpdate;
            newArr[i].certiRank = rankCertificateUpdate;
            newArr[i].certiDate = dateCertificateUpdate;
          }
        }
        setListCertificate(newArr);
      },
      (error) => {
        const message =
          (error.response && error.response.data) ||
  ...
  return (
    <div>
      <section class="my-5">
        ...
        <div bg-white shadow rounded-lg d-block d-sm-flex>
          {isShowAdd ? (
            <a
              className="btn btn-default btn-icon-split"
