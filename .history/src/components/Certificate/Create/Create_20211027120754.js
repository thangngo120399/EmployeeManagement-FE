...
function Create(props) {
  const { candidateName, idCandidate, setListCertificate, listCertificate } =
    props;
  const form = useRef();
  ...
  const handleCreate = (e) => {
    ...
    if (checkBtn.current.context._errors.length === 0) {
      ...
      arrCertificate.push({
        ...
        certificateDate: format(certifiCateDate, "yyyy-MM-dd"),
      });
    }
  };
  // const deleteItem = (id) => {
  ...
  const addCertificate = (item) => {
    CertificateService.createCertificated(
      ...
      (response) => {
        setCertifiCateDate([...listCertificate, item]);
      },
      (error) => {
        const message =
