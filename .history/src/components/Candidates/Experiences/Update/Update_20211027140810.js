...
function Update(props) {
  ...
  const [experienceYears, setExperienceYears] = useState(item.experInYear);
  const [dob, setDob] = useState(new Date(item.dob));
  const [phone, setPhone] = useState(item.phone);
  const [email, setEmail] = useState(item.email);
  const [skills, setSkills] = useState(item.skills);
  const [idCandidate, setIdCandidate] = useState(item.idCandidate);
  ...
  const handleAfterDelete = () => {
    CandidateService.deleteCandidate(idCandidate).then(
      (response) => {
        toast.success("Delete Successfully");
        props.history.push("/recruiment");
      },
      (error) => {
        const message =
          (error.response && error.response.data) ||
