...
const InforCard = (props) => {
  const { item } = props;
  const [intern, setIntern] = useState(item);
        ...
        <div className="row no-gutters">
          ...
          <div className="col-md-8">
            <div
              ...
              onClick={() =>
                history.push({
                  pathname: `/intern/profile/${intern.id}`,
                  // state: { intern },
                })
              }
            >
              <div className={intern.status ? "green" : "red"}></div>

              <h5 className="card-title one-line">
                <b>{intern.fullName}</b>
              </h5>
              <ul>
                <li>{intern.age} ages</li>
                <li>
                  <i>{intern.positionName}</i>
                </li>
              </ul>
            </div>
          </div>
